...
import { CheckCircle2, Download, Printer, Home, Ticket } from 'lucide-react';
  ...
  if (!booking) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-24 text-center space-y-4">
        ...
        <p className="text-muted-foreground text-sm font-semibold">Generating Digital Cinema E-Ticket...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12 space-y-8">
      ...
      <div className="text-center space-y-3">
        ...
        <h1 className="text-3xl font-black tracking-tight">Reservation Confirmed!</h1>
        <p className="text-sm text-muted-foreground">
          Your e-ticket has been generated. Present the QR code upon entering the cinema hall.
        </p>
      </div>

      {/* Printable Cinema Ticket Card */}
      <div id="printable-ticket" className="bg-card border-2 border-border rounded-3xl overflow-hidden shadow-2xl space-y-0 divide-y divide-border">
        {/* Header Strip */}
        <div className="p-6 bg-gradient-to-r from-primary to-rose-700 text-white flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Ticket className="w-6 h-6" />
            <span className="font-extrabold text-lg tracking-wider">OLYMPUS DIGITAL TICKET</span>
          </div>
          <span className="text-xs font-mono font-bold px-3 py-1 bg-black/30 rounded-full">
            {booking.bookingCode}
          </span>
        </div>
        ...
        <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              <div>
                <span className="text-muted-foreground block">Auditorium</span>
                <strong className="text-foreground text-sm">{booking.hallName}</strong>
              </div>
              <div>
                <span className="text-muted-foreground block">Format</span>
                <strong className="text-foreground text-sm">{booking.screenType}</strong>
              </div>
              <div>
                <span className="text-muted-foreground block">Date & Time</span>
                <strong className="text-foreground text-sm">{formatDate(booking.date)} at {booking.startTime}</strong>
              </div>
              <div>
      ...
      <div className="flex flex-wrap items-center justify-center gap-4 pt-4">
        ...
        <button
          onClick={() => alert('E-Ticket downloaded as PDF (Simulated).')}
          className="flex items-center gap-2 px-6 py-3 bg-secondary text-secondary-foreground font-bold rounded-xl text-xs hover:bg-secondary/80 transition-colors border border-border"
        >
        ...
        <Link
          href="/"
          className="flex items-center gap-2 px-6 py-3 bg-primary text-white font-bold rounded-xl text-xs hover:bg-primary/90 transition-colors shadow-lg shadow-primary/30"
        >
