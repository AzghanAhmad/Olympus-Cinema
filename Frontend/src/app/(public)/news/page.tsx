...
export default function NewsPage() {
  ...
  useEffect(() => {
    newsService.getNews().then((list) => {
      setArticles(list);
      setLoading(false);
    });
  }, []);

  const featuredArticle = articles.find((n) => n.isFeatured) || articles[0];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Articles published from the admin news panel.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="p-8 text-center bg-card border border-border rounded-2xl text-sm text-muted-foreground">
            No news articles yet. Publish stories in the admin panel to display them here.
          </div>
        ) : (
          <>
            {featuredArticle && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {articles.map((article) => (
                <div
                  key={article.id}
                  className="bg-card border border-border rounded-2xl overflow-hidden flex flex-col justify-between shadow-sm"
                >
